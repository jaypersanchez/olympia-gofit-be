...
import userWorkoutWeek from '../models/userWorkoutWeek.js';
...
const createWeeklyWorkoutPlan = async (user) => {
  ...
  if (!isDateOlderThan16Years(age)) {
    ...
    for (let i = 0; i < frequency; i++) {
      ...
      let exercises = await Exercise.find({category: category, level: {$nin: notInclude}});
...
async function createFullWorkoutPlan(user) {
  ...
  const savedWeeklyPlans = await Promise.all(weeklyPlans.map(async (plan, idx) => {
    // creates a new instance of the userWorkoutWeek model and saves it to the database
    let newWeek = new userWorkoutWeek({ user: user._id, workouts: plan, workoutsPerWeek: user.frequency , weekNumber: idx + 1});
