...
const userSchema = new mongoose.Schema({
    ...
    gymType: { type: String, enum: ["home", "commercial", "small", "crossfit"], default: "home"}, 
    frequency: { type: Number},
    caloriesBurned:[{ type: Number}],
    customerId: { type: String, default: null},
    paymentSchedule: { type: String, enum: ["month", "year", "trial"], default: "month"},
    paymentActive: { type: Boolean, default: false},
    workoutPlans: [{ type: mongoose.Schema.Types.ObjectId, ref: "UserWorkoutPlan" }],
});
...
userSchema.static("findUsers", async function (query) {
  ...
  userSchema.statics.checkCredentials = async function (email, plainPW) {
    ...
    //finds user by email
    //if user => compare PWs 
    const user = await this.findOne({email: email}).populate({path: "workoutPlans", populate:{path: "weeks", populate: {path:"workouts.exercises.exercise"}} })
